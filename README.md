...
## Relationship to Karpathy-Inspired Claude Code Guidelines

AOK's global Codex instruction template includes four coding-agent behavior principles adapted from [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills): Think Before Coding, Simplicity First, Surgical Changes, and Goal-Driven Execution.

That repository is a community project derived from Andrej Karpathy's public observations on LLM coding pitfalls. It is not treated as an official Karpathy project. AOK uses the principles as optional global behavior guidance, while project-specific `AGENTS.md` files should still focus on concrete project context, commands, verification routing, and document routing.
...
- 기획만 있는 repo에서 개발을 시작하려는데 agent가 무엇부터 봐야 할지 모릅니다.
- 기능 추가, 버그 수정, 리팩터링, 문서 변경의 작업 파이프라인이 매번 흔들립니다.
- agent가 문서를 많이 만들었지만 코드와 문서, 문서와 문서 사이의 기준이 어긋납니다.
- `AGENTS.md`나 설계 문서가 너무 커져서 작은 작업에도 컨텍스트를 많이 소모합니다.
- 기존 프로젝트에 agent 운영 규칙을 도입하고 싶지만 기존 문서를 잃거나 덮어쓸 위험이 있습니다.

AOK는 이런 상황에서 프로젝트별 agent 지침, workflow, 문서 지도, 문서 거버넌스 규칙을 작고 명확한 파일로 나누어 적용하게 합니다.

## 핵심 원칙

- 루트 `AGENTS.md`는 매 세션 읽어도 부담 없는 크기로 유지합니다. 작은 프로젝트는 50-100줄 안팎을 우선합니다.
- 프로젝트에 필요한 구성요소만 선택해 적용합니다.
- 사용자에게 필요한 적용 범위를 선택지로 먼저 묻습니다.
- 기존 프로젝트 문서는 먼저 읽고, 정보 손실 없이 AOK 구조에 반영합니다.
- 기존 문서 이동, 삭제, deprecated/archive 처리는 사용자 승인 후 진행합니다.
- 긴 절차는 Superpowers를 사용하는 프로젝트에서만 `.agents/workflows/*.md`로 분리합니다.
- Superpowers를 쓰지 않는 프로젝트에는 AOK workflow 문서를 제공하지 않고, 루트 `AGENTS.md`의 기본 작업 원칙만 적용합니다.
- `~/.codex/AGENTS.md` 같은 전역 지침은 기본 적용 대상이 아니며, 사용자가 선택한 경우에만 다룹니다.

AOK 적용의 성공 기준은 일반 작업에서 agent가 읽어야 하는 기본 컨텍스트가 줄어드는 것입니다. AOK를 적용한 결과 모든 작업에서 더 많은 문서를 기본으로 읽게 된다면 적용이 잘못된 것입니다.
...
AOK의 선택 `DESIGN.md` 템플릿은 [Google Stitch](https://stitch.withgoogle.com/docs/design-md/specification)가 도입하고 [google-labs-code/design.md](https://github.com/google-labs-code/design.md)에서 명세한 DESIGN.md 형식을 따릅니다.

AOK의 템플릿은 AOK의 필요한 문서만 읽는 모델에 맞춘 적용 템플릿입니다. 공식 Google 템플릿은 아니며, 제3자 `DESIGN.md` 예시는 프로젝트 소유자가 공식 디자인 시스템이라고 확인한 경우가 아니라면 참고 자료로 취급합니다.

## Karpathy 기반 Claude Code 지침과의 관계

AOK의 전역 Codex 지침 템플릿은 [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills)에서 정리한 네 가지 코딩 agent 행동 원칙을 포함합니다. 네 원칙은 Think Before Coding, Simplicity First, Surgical Changes, Goal-Driven Execution입니다.

해당 저장소는 Andrej Karpathy의 공개적인 LLM 코딩 실패 관찰을 바탕으로 만든 커뮤니티 프로젝트이며, Karpathy의 공식 프로젝트로 취급하지 않습니다. AOK는 이 원칙을 선택적인 전역 행동 지침으로 사용하고, 프로젝트별 `AGENTS.md`는 여전히 구체적인 프로젝트 맥락, 명령어, 검증 라우팅, 문서 라우팅에 집중합니다.
